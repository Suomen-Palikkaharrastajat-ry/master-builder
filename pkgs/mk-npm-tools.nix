# Shared builder for the app repos' npm tool sets (vite, elm-test,
# elm-tailwind-classes), packaged from each repo's own package-lock.json so the
# versions stay in sync with its package.json.
#
# Consumers (planet, event-calendar, service-map) call this from their
# pkgs/npm-tools.nix with their repo name, their pkgs/ directory as `src`,
# and their fetchNpmDeps hash:
#
#   { pkgs }:
...
#   1. Set hash = pkgs.lib.fakeHash; in the consumer
#   2. Run `devenv shell` — the build fails with the correct sha256 in "got:"
#   3. Paste that sha256 into the consumer
{
  pkgs,
  name,
  src,
...
let
  ...
  patchedSrc =
    pkgs.runCommand "${name}-npm-src" { nativeBuildInputs = [ pkgs.jq ]; }
      ''
        mkdir $out
        jq 'del(.scripts.postinstall)' ${src}/package.json > $out/package.json
